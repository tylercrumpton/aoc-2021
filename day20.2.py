...
def get_index(input_image, coord):
    ...
    for affected in get_affected_coords(coord):
        ...
        # else:
        binary_string += str(input_image.get(affected, 1 if flasher_state else 0))
...
def process_input_image(input_image):
    ...
    for coord in input_image.keys():
        coords_to_process = coords_to_process.union(set(get_affected_coords(coord)))
    for coord in coords_to_process:
        index = get_index(input_image, coord)
        if decoder[index] == "#":
            output_image[coord] = 1
        else:
            output_image[coord] = 0
...
print(len([i for i in input_image.values() if i == 1]))
