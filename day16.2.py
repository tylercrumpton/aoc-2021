...
def parse_pkt(pkt):
    ...
    if type == 4:  # Literal
        ...
        match (type):
            case 0:  # sum
                value = sum(sub[2] for sub in sub_pkts)
            case 1:  # product
                value = 1
                for sub in sub_pkts:
                    value *= sub[2]
            case 2:
                value = min(sub[2] for sub in sub_pkts)
            case 3:
                value = max(sub[2] for sub in sub_pkts)
            case 5: #>
                value = 1 if sub_pkts[0][2] > sub_pkts[1][2] else 0
            case 6:
                value = 1 if sub_pkts[0][2] < sub_pkts[1][2] else 0
            case 7:
                value = 1 if sub_pkts[0][2] == sub_pkts[1][2] else 0
